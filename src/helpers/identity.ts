...
export class Identity {
  ...
  async makeAuthResponse({
    ...
    if (scopes.includes('publish_data')) {
      ...
      const filePath = this.gaiaReadUrlToWritePath(
        profileUrl,
        hubInfo.read_url_prefix,
        address,
      );
  ...
  appPrivateKey(appDomain: string) {
    ...
    const appsNode = new IdentityAddressOwnerNode(
      bip32.fromBase58(appsNodeKey),
      salt,
    );
