...
export const selectFirstIdentity = (state: AppState) =>
  state.wallet.currentWallet?.identities[0];
export const selectIsSignedIn = (state: AppState) =>
  state.wallet.identities?.length > 1;
export const selectIsRestoringWallet = (state: AppState) =>
